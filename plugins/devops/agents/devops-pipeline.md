...
## STEP_0 — project-context チェック (必ず最初に実行)

```
Glob: project-context/structure.md
```

- **存在する** → そのまま STEP_MODE へ進む
- **存在しない** → ユーザーに選択肢を提示:

```
## ⚠️ project-context が見つかりません

プロジェクトコンテキストがないと、スキルが最適な結果を出せません。

**A) 推奨: project-onboarding を先に実行**
   プロジェクト構造を自動分析してコンテキストを生成します。
   → "project-onboarding 실행해줘" と言ってください

**B) このまま続ける**
   コンテキストなしで進みます（品質が下がる可能性があります）

どちらにしますか？
```

ユーザーが **B** を選択した場合 → STEP_MODE へ進む
