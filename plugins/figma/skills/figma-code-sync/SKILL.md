...
# Figma Code Sync — Design Implementation Verifier

Figmaデザインとコード実装の一致を検証するスキル。
マッピング表と実装コードを照合し、抜けや乖離を検出する。
...
If no mapping table exists → stop and instruct:
> "先に `figma-framework-figma-mapper` スキルを実行してマッピング表を生成してください。"
...
If no token file exists → flag as WARN (continue, but note the gap).

---

## Step 3 — Scan Implementation Code
...
- [ ] Are any hardcoded color values present that should use tokens?
  - Pattern: `#[0-9a-fA-F]{3,6}` — flag all hardcoded hex colors
  - Pattern: `rgba?\([^)]+\)` — flag hardcoded rgba/rgb
  - Pattern: `font-size:\s*\d+px` — flag hardcoded font sizes
  - Pattern: `padding:\s*\d+px` — flag hardcoded pixel values
...
|-----------------|--------|-----------------|
| Modal/Confirmation | ❌ Not implemented | Create ConfirmModal component |
| Toast/Error | ❌ Not implemented | Create Toast component with error variant |

### ⚠️ Token Violations (Hardcoded Values)
...
**Sync Score: 75%** — 実装は概ね完了しているが、未実装コンポーネントとトークン違反を修正してください。
```
...
1. [MUST] Missing components → implement before release
   - Modal/Confirmation
   - Toast/Error

2. [MUST] Token violations → replace hardcoded values with tokens
   - src/components/Button.tsx line 24: #3B82F6 → var(--color-primary-500)
   - src/components/Card.tsx line 8: 16px → var(--spacing-4)

3. [SHOULD] Props gaps → add missing variants
...
### With devops-pipeline (Step 5 — Frontend Review)
...
- `figma-code-sync` → structural check (components, tokens, props) ← **this skill**
- `devops-frontend-review` → visual check (screenshots, pixel comparison)
...
| 70–89% | 🟡 WARN | Fix before release |
| < 70% | 🔴 FAIL | Significant work needed |
